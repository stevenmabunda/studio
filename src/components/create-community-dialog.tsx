...
export function CreateCommunityDialog({
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await createCommunity(user.uid, formData);
      if (result.success) {
        ...
        toast({ variant: 'destructive', description: result.error || 'Failed to create community.' });
      }
    } catch (error) {
      ...
      <DialogContent>
        ...
        <form ref={formRef} onSubmit={handleFormSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            >
              {profilePicPreview ? (
                <Image src={profilePicPreview} alt="Community preview" layout="fill" objectFit="cover" className="rounded-md" />
              ) : (
